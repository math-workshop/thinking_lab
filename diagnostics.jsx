...
function DiagnosticPage({ role }) {
  ...
  if (tool === 'essay') return <window.MMM_QUAL_DIAG.EssayInstrument onHome={()=>setTool(null)} role={role}/>;
  if (tool === 'portfolio') return <window.MMM_QUAL_DIAG.PortfolioInstrument onHome={()=>setTool(null)} role={role}/>;
  ...
  const cards = [
    ...
    {
      id:'essay', t:'Эссе', sub:'Финальное эссе',
      name:'Финальное эссе «Моя любимая задача»',
      desc:'Качественный инструмент итоговой диагностики (К5). Три параметра по шкале 1–3, максимум 9 баллов.',
      meta:['К5 · рефлексия','3 параметра · 1–3','Оценивает педагог'],
      filler:'У педагога', color:'#9a4a6a',
    },
    {
      id:'portfolio', t:'Портфолио', sub:'Учебные достижения',
      name:'Портфолио учебных достижений',
      desc:'Текущий мониторинг динамики: ведётся весь год, оценивается дважды. Четыре критерия 1–3, максимум 12 баллов.',
      meta:['Динамика · К5','4 критерия · 1–3','Оценивает педагог'],
      filler:'У педагога', color:'#4a6a8a',
    },
  ];
  ...
  return (
    <div style={{ maxWidth: 1080, margin:'0 auto', display:'flex', flexDirection:'column', gap: 'var(--pad-lg)' }}>
      <header>
        <p className="mmm-eyebrow">Диагностика курса</p>
        <h1 className="mmm-h1">Инструменты диагностики</h1>
        <p className="mmm-lead">Входная (ДТМ, ОПМ) — в сентябре; текущая (КПН, портфолио) — в течение года; итоговая (финальное эссе) — в конце. Вместе покрывают пять компонентов мышления (К1–К5).</p>
      </header>
