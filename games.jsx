      ...
      <div style={{ display:'flex', gap:4, justifyContent:'center', flexWrap:'wrap' }}>
        {Array.from({length: data.size}, (_,i)=>i+1).map(n => (
          <button key={n} className="mmm-btn ghost" onClick={()=>setCell(n)}
            style={{minWidth: 32, padding:'6px 0', fontFamily:'var(--serif)', fontSize:16}}>
            {n}
          </button>
        ))}
        <button className="mmm-btn ghost" onClick={()=>setCell(0)} title="Стереть">×</button>
      </div>
...
function MagicSquareGame() {
  ...
  return (
    <div className="mmm-card" style={{ display:'flex', flexDirection:'column', gap:12 }}>
      ...
      <div style={{ padding:14, background:'var(--paper-2)', borderRadius:8, border:'1px dashed var(--line)' }}>
        <div style={{
          display:'grid', gridTemplateColumns:'28px repeat(3, 50px) 30px', gap:0,
          width:'fit-content', margin:'0 auto', alignItems:'center',
        }}>
          {/* верхний ряд: пусто + суммы столбцов + диагональ ↗ */}
          <div/>
          {sums.c.map((s,i)=> (
            <div key={'c'+i} style={{fontSize:12, color: lineColor(s, colFilled(i)), textAlign:'center', padding:4, fontWeight:600, fontFamily:'var(--mono)'}}>
              {s||'·'}
            </div>
          ))}
          <div style={{fontSize:11, color: lineColor(sums.d2, d2Filled), textAlign:'center', padding:4, fontWeight:600, fontFamily:'var(--mono)'}}>
            ↗{sums.d2||0}
          </div>
          {/* строки */}
          {grid.map((row,r) => (
            <React.Fragment key={r}>
              <div style={{fontSize:12, color: lineColor(sums.r[r], rowFilled(r)), padding:4, fontWeight:600, fontFamily:'var(--mono)', textAlign:'right'}}>
                {sums.r[r]||'·'}
              </div>
              {row.map((v,c) => {
                const isSel = sel && sel[0]===r && sel[1]===c;
              ...
              {/* диагональ ↘ — справа от средней строки */}
              {r===1 && (
                <div style={{
                  gridRow: '2 / span 3', gridColumn: 5, alignSelf:'center', textAlign:'center',
                  fontSize:11, color: lineColor(sums.d1, d1Filled), padding:4, fontWeight:600, fontFamily:'var(--mono)',
                }}>
                  ↘{sums.d1||0}
                </div>
              )}
        ...
        {[1,2,3,4,5,6,7,8,9].map(n => {
          ...
          return (
            <button key={n} className="mmm-btn ghost"
              ...
              style={{
                minWidth:34, padding:'6px 0', fontFamily:'var(--serif)', fontSize:16,
